...
<script lang="ts">
	...
	// On server: use light (will be replaced immediately on client)
	// On client: read from HTML data-theme attribute (set by blocking script)
	let logoSrc = $state('/icons/Artellico Logo.png');
	
	// Update logo when theme changes or on mount
	$effect(() => {
		if (!browser) return;
		
		// Get current theme from DOM (set by blocking script or store)
		const currentTheme = document.documentElement.getAttribute('data-theme');
		const isDark = currentTheme === 'dark' || themeStore.effectiveTheme === 'dark';
		logoSrc = isDark ? '/icons/Artellico Logo White.png' : '/icons/Artellico Logo.png';
	});
...
<div class="app-layout" class:reading-mode={readingMode.enabled}>
	...
	<!-- ==========================================
	     SIDEBAR
	     Contains: Logo, Navigation, Copyright
	     ========================================== -->
	<aside class="sidebar hide-mobile">
		<!-- Sidebar Header with Logo -->
		<div class="sidebar-header">
			<a href="/" class="sidebar-brand">
				<img src={logoSrc} alt="Artellitext" class="sidebar-logo" />
		...
		<header class="main-header">
			...
			<div class="header-controls">
				...
				<ThemeToggle
					...
					onToggle={themeStore.toggleTheme}
				/>
...
<style>
	/* Layout is defined in app.css, this is for component-specific overrides */
	...
	.app-layout.reading-mode .sidebar,
