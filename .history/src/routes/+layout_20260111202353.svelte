...
<script lang="ts">
	...
	// Get initial theme from HTML attribute (set by blocking script)
	function getInitialTheme(): 'light' | 'dark' {
		if (!browser) return 'light';
		return (document.documentElement.getAttribute('data-theme') as 'light' | 'dark') || 'light';
	}
	
	// Logo source based on theme - uses HTML attribute initially to avoid flash
	let logoSrc = $derived(
		(browser ? getInitialTheme() : 'light') === 'dark' || themeStore.effectiveTheme === 'dark'
