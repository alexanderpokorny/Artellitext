...
export function createTheme(initial?: Theme) {
	...
	// Use $effect.pre to set up listener without triggering theme change on mount
	$effect(() => {
		if (!browser || theme !== 'auto') return;
		
		const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
		
		// Only handle CHANGES, not initial state
		const handler = (e: MediaQueryListEvent) => {
			// System preference changed while in auto mode
			applyTheme('auto');
		};
