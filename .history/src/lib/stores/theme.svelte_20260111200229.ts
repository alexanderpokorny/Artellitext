...
export function createTheme(initial?: Theme) {
	// Read the already-applied theme from HTML (set by blocking script in app.html)
	// This prevents theme flash by not re-applying on initial load
	const htmlTheme = browser ? document.documentElement.getAttribute('data-theme') as 'light' | 'dark' : null;
	...
	// Apply theme on change - but skip first render since HTML script already applied it
	$effect(() => {
		if (isFirstRender) {
			isFirstRender = false;
			return;
