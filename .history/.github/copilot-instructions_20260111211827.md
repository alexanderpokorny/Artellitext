...
- **Kein Inline-CSS** in Svelte-Komponenten (ausgenommen: einfache Layout-Positionierung)
...
- Theme-basierte Selektoren (`[data-theme="dark"]`) in app.css
- Svelte-Komponenten dürfen CSS-Klassen aus app.css verwenden

#### Ladeverhalten (KRITISCH):
1. **Zuerst**: `theme-init.js` lädt (setzt `data-theme` Attribut)
2. **Dann**: `app.css` lädt (enthält ALLE Styles)
3. **Zuletzt**: Svelte-Komponenten rendern

#### Theme-Initialisierung:
- Theme-Erkennung erfolgt in `/static/js/theme-init.js`
- Script muss VOR dem CSS geladen werden
- Setzt `data-theme` Attribut auf `<html>` Element
- Kein Inline-JavaScript in app.html
