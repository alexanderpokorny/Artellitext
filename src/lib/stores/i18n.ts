...
/**
 * Create i18n state with Svelte 5 runes.
 * Returns reactive state and helper functions.
 */
export function createI18n(initial?: SupportedLanguage) {
	// Determine initial language
	const initialLang = initial || getStoredLanguage() || detectBrowserLanguage();
	
	// Reactive language state
	let language = $state<SupportedLanguage>(initialLang);
